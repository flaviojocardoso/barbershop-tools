// script.js - Versão Original Ajustada (Correção Relatório + Exportação TXT)
// Mantendo toda estrutura, keys e save originais.
...
function initApp() {
    ...
    // Exportar - ALTERADO APENAS AQUI PARA CHAMAR A FUNÇÃO TXT
    const btnPdf = document.getElementById('btnExportPdf');
    if(btnPdf) btnPdf.onclick = () => { exportTxt(); };
...
// --- AUXILIAR: Filtro Seguro de Data (Mês/Ano Local) ---
function isCurrentMonth(dateStr) {
    if(!dateStr) return false;
    const now = new Date();
    const [y, m, d] = dateStr.split('-').map(Number);
    // m na string é 1-12, getMonth é 0-11
...
function renderMonthSummary() {
    ...
    // FIX: Usar isCurrentMonth em vez de startsWith(ym)
    const entradasMes = store.entradas.filter(e => isCurrentMonth(e.data));
    const gastosMes = store.gastos.filter(g => isCurrentMonth(g.data));
...
function renderReportWeeks(){
  const now = new Date();
  // FIX: Passar Ano e Mês (1-12) local
  const weeks = getWeeksForMonth(now.getFullYear(), now.getMonth() + 1);
  const rw = document.getElementById('reportWeeks');
  if(!rw) return;
  rw.innerHTML = '';
  
  weeks.forEach((w, idx) => {
    // Converter datas da semana para string YYYY-MM-DD local
    const dateKeys = w.dates.map(d => {
...
function renderReportDays(){
  ...
  for(let d=1; d<= lastDay; d++){
    // FIX: Gerar string local, sem UTC
...
// --- NOVA FUNÇÃO DE EXPORTAÇÃO (MÊS ANTERIOR) ---
function exportTxt() {
    const now = new Date();
    ...
    // Ajuste de virada de ano (Se for Janeiro(0) - 1 = -1 -> vira Dezembro do ano passado)
    if (targetMonth < 0) {
        targetMonth = 11;
        targetYear--;
    }

    // Filtro específico para o mês/ano calculado
    const isTargetDate = (dateStr) => {
        if(!dateStr) return false;
        const [y, m, d] = dateStr.split('-').map(Number);
        // m vem do string (1-12), subtraímos 1 para comparar (0-11)
        return y === targetYear && (m - 1) === targetMonth;
    };

    const ents = store.entradas.filter(e => isTargetDate(e.data));
    const gas = store.gastos.filter(g => isTargetDate(g.data));

    // Validação: Se não houver dados, exibe alerta e para
    if (ents.length === 0 && gas.length === 0) {
        alert(`Nenhum dado encontrado para o mês anterior (${targetMonth + 1}/${targetYear}).`);
        return;
    }

    const header = `RELATÓRIO FINANCEIRO - ${targetMonth + 1}/${targetYear}\nGerado em: ${now.toLocaleString('pt-BR')}\n----------------------------------\n`;
    
    let content = header + "\n[ENTRADAS]\n";
    let totE = 0;
    ents.forEach(e => {
        totE += e.valor;
        content += `${e.data} | ${e.desc} | ${e.categoria} | ${formatCurrency(e.valor)}\n`;
    });
    ...
    gas.forEach(g => {
        totG += g.valor;
        content += `${g.data} | ${g.desc} | ${g.categoria} | ${formatCurrency(g.valor)}\n`;
    });

    const diz = totE * TITHE;
    const liq = (store.saldoInicial||0) + totE - totG - diz;

    content += "\n----------------------------------\n[RESUMO]\n";
    content += `Entradas: ${formatCurrency(totE)}\n`;
    content += `Saídas:   ${formatCurrency(totG)}\n`;
    content += `Dízimo:   ${formatCurrency(diz)}\n`;
    content += `Inicial:  ${formatCurrency(store.saldoInicial||0)}\n`;
    content += `FINAL:    ${formatCurrency(liq)}\n`;

    const blob = new Blob([content], { type: 'text/plain' });
    const url = window.URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = `Relatorio_${targetYear}_${targetMonth + 1}.txt`;
    a.click();
